      ...
      <KeyboardAvoidScrollview
        ...
        <View style={styles.taskContainer}>
          {tasks?.map((item, index) => (
            <View key={index} style={{ ...styles.inputContainer, borderBottomWidth: index === tasks?.length - 1 ? 0 : StyleSheet.hairlineWidth }}>
              <PrimaryInput
        ...
        <Row style={styles.row}>
          ...
          <InputWithIcon
            containerStyle={styles.width}
            value={reminderTypes?.find(x => x?.id === payload?.reminderType)?.title || ''}
            id={payload?.reminderType}
            label='Select Reminder'
            onChangeText={(v) => setPayload({ ...payload, reminderType: v })}
            items={reminderTypes} icon={'down'} />
        </Row>
        <Medium label={'Select Task Type'} style={{
        ...
        <Row style={{ marginBottom: mvs(20) }}>
          <PrimaryButton
            ...
            textStyle={{ color: payload?.taskType !== 'One Time' ? colors.black : colors.white }}
            containerStyle={{
              width: '50%',
              borderRadius: 0,
              borderTopLeftRadius: mvs(10),
              backgroundColor: payload?.taskType == 'One Time' ? colors.primary : colors.secondary
            }}
          />
          <PrimaryButton
            title={t('Recurring')}
        ...
        {payload?.taskType !== 'One Time' && <InputWithIcon
          containerStyle={{ marginBottom: mvs(20) }}
          value={repeatTypes?.find(x => x?.id === payload?.repeatType)?.title || ''}
          id={payload?.repeatType}
          label='Repeat After'
          onChangeText={(v) => setPayload({ ...payload, repeatType: v })}
          items={repeatTypes} icon={'down'} />}
        <PrimaryButton
          loading={loading}
          title={t('Update')}
